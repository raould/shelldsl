...
RULE = add_rule(Severity.ERROR, "with statement", ["try and finally"])
...
def check_source(source: str, filename: str = "<string>") -> List[Diagnostic]:
    ...
    if tree is None:
        return []
    return [
        make_diagnostic(filename, node_position(node)[0], node_position(node)[1], RULE)
